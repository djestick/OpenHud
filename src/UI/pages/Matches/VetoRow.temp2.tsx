...
export const VetoRow: React.FC<VetoRowProps> = ({
  ...
  const handleScoreChange = (teamId: string | null, rawValue: string): void => {
    if (!teamId) return;
  ...
  const handleWinnerChange = (teamId: string | null): void => {
    if (!teamId) return;
    onVetoChange(index, "winner", teamId);
  };

  const handleMapEndToggle = (checked: boolean): void => {
